      ...
      <section className="grain relative overflow-hidden bg-canvas px-5 pb-16 pt-36 sm:px-8 sm:pb-20 sm:pt-44">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-muted">
            Studio One est pensé pour fonctionner avec des comptes de démonstration. Voici ce que nous faisons, et ce que
            nous ne faisons pas, de vos accès et de vos données.
          </p>
        </div>
      </section>
      ...
      <section className="border-t border-hairline bg-canvas-soft">
        <div className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-28">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {POINTS.map((p) => (
              <FeatureCard key={p.title} icon={p.icon} title={p.title} text={p.text} accent={ACCENT} />
            ))}
          </div>
          ...
          <div className="card mt-10 p-7">
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted">
              Studio One ne promet pas une sécurité absolue : c'est un outil de production vidéo. Pour les démonstrations,
              nous recommandons toujours un compte dédié, sans données clients réelles, et de vérifier les écrans avant
              diffusion.
            </p>
          </div>
