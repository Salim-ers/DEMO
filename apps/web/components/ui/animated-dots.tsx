...
export const AnimatedDots: React.FC<AnimatedDotsProps> = ({
  ...
  const dotsRef = useRef<{ draw: () => void }[]>([]);
  ...
  useEffect(() => {
    ...
    class Dot {
      ...
      updateColors(selectedColor: [string, number, number, number] | undefined, increment: number) {
        if (!selectedColor) return "rgba(255,255,255,1)";
