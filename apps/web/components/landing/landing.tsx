...
import {
  ...
  ShieldCheck, Rocket, Briefcase, Building2, Megaphone, Code2,
} from "lucide-react";
  ...
  return (
    <>
      ...
      <section id="presentation" className="relative flex min-h-screen items-center justify-center overflow-hidden bg-canvas">
        ...
        <div aria-hidden className="pointer-events-none absolute inset-0 z-0" style={{ background: "radial-gradient(46% 48% at 50% 48%, rgba(6,5,4,0.74) 0%, rgba(6,5,4,0.42) 50%, transparent 80%)" }} />

        <HeroStagger className="relative z-10 flex max-w-3xl flex-col items-center px-5 py-28 text-center">
          <HeroItem>
            <LogoMark tone="cream" size={124} className="mb-7 max-w-[46vw] drop-shadow-[0_18px_50px_rgba(0,0,0,0.55)]" />
          </HeroItem>
          <HeroItem>
            ...
            <ul className="mt-7 flex flex-col items-center gap-x-6 gap-y-2 text-sm text-muted sm:flex-row">
              {HERO_POINTS.map((p) => (
                <li key={p} className="flex items-center gap-2">
                  <Check size={16} className="text-accent-deep" /> {p}
                </li>
              ))}
            </ul>
          </HeroItem>
      ...
      {/* 2 — POURQUOI */}
      <section id="pourquoi" className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-28">
        <Reveal>
      ...
      <section id="fonctionnement" className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-28">
        <Reveal>
          <SectionHeader align="center" eyebrow="Comment ça marche" title="Comment ça marche" subtitle="Un processus simple, pensé pour aller de votre produit à une vidéo prête à partager." className="[&_p.max-w-2xl]:mx-auto" />
        </Reveal>
